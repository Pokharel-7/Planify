...
const inviteMember = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    if (!user) {
      ...
      console.log('[InviteMember] Created new account for invited user:', user.email);
    }
    ...
    try {
      ...
      console.log('[InviteMember] isNewAccount:', isNewAccount, 'tempPassword set:', !!tempPassword);
      if (isNewAccount && tempPassword) {
        console.log('[InviteMember] Sending temp password email to:', user.email);
        await emailService.sendTempPasswordInvitation({
          recipientEmail: user.email,
          recipientName: user.name,
          inviterName: inviter?.name || 'A team member',
          workspaceName: workspace.name,
